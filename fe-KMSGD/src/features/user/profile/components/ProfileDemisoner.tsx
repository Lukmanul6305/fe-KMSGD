import { useState } from "react";
import { ChevronDown, User } from "lucide-react";
import { profileDemisioner } from "../services/profileService";
import MemberCard from "./MemberCard";

export default function ProfileDemisioner() {
    const [openIndex, setOpenIndex] = useState<number | null>(null);

    // show all anggota per periode
    const [showAllMembers, setShowAllMembers] = useState<number | null>(null);

    const toggle = (index: number) => {
        setOpenIndex(openIndex === index ? null : index);
    };

    return (
        <section className="py-20 px-15 border-t border-[#2a2a2a]">

            {/* TITLE */}
            <h2 className="text-3xl md:text-4xl font-bold font-['Montserrat'] text-[#ffd700] mb-3">
                <span className="text-white">Demisioner</span> Pengurus
            </h2>

            <p className="text-[#8b8b8b] text-sm mb-10">
                Kumpulan data kepengurusan dari tahun ke tahun yang telah
                menyelesaikan masa baktinya dengan penuh dedikasi.
            </p>

            {/* LIST */}
            <div className="space-y-5">

                {profileDemisioner.map(({ periode, anggota }, i) => {

                    const isOpen = openIndex === i;

                    // tampil 10 dulu
                    const visibleMembers =
                        showAllMembers === i
                            ? anggota
                            : anggota.slice(0, 10);

                    return (
                        <div
                            key={i}
                            className={`bg-[#111] border transition-all duration-300 ${isOpen
                                ? "border-[#2c2c2c]"
                                : "border-[#1f1f1f] hover:border-[#2a2a2a]"
                                }`}
                        >

                            {/* HEADER */}
                            <button
                                onClick={() => toggle(i)}
                                className="w-full flex justify-between items-center p-6 text-left"
                            >
                                <div className="flex items-center gap-5">

                                    {/* ICON */}
                                    <div
                                        className={`w-14 h-14 flex items-center justify-center border transition-all ${isOpen
                                            ? "bg-[#ffd700] border-[#ffd700]"
                                            : "bg-[#151515] border-[#2a2a2a]"
                                            }`}
                                    >
                                        <User
                                            size={22}
                                            className={
                                                isOpen
                                                    ? "text-black"
                                                    : "text-[#ffd700]"
                                            }
                                        />
                                    </div>

                                    {/* TEXT */}
                                    <div>

                                        <h3
                                            className={`text-xl font-bold font-['Montserrat'] ${isOpen
                                                ? "text-white"
                                                : "text-[#f5f5f5]"
                                                }`}
                                        >
                                            {periode}
                                        </h3>

                                        <p className="text-[#777] text-sm mt-1">
                                            Kabinet Harmoni Sinergi •{" "}
                                            {anggota.length} Anggota
                                        </p>

                                    </div>
                                </div>

                                {/* DROPDOWN */}
                                <span
                                    className={`text-[#777] transition-transform duration-300 ${isOpen ? "rotate-180" : ""
                                        }`}
                                >
                                    <ChevronDown size={20} />
                                </span>
                            </button>

                            {/* CONTENT */}
                            <div
                                className={`overflow-hidden transition-all duration-500 ${isOpen
                                    ? "max-h-500 opacity-100 px-6 pb-6"
                                    : "max-h-0 opacity-0"
                                    }`}
                            >

                                {/* CARD MEMBER */}
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                                    {visibleMembers.map(({ jabatan, nama }, idx) => (
                                        <MemberCard
                                            key={idx}
                                            member={{ nama, jabatan }}
                                            variant="demisioner"
                                        />
                                    ))}
                                </div>

                                {/* BUTTON SHOW MORE */}
                                {anggota.length > 10 && (
                                    <div className="flex justify-center mt-6">

                                        <button
                                            onClick={() =>
                                                setShowAllMembers(
                                                    showAllMembers === i
                                                        ? null
                                                        : i
                                                )
                                            }
                                            className="text-xs text-[#b0b0b0] hover:text-[#ffd700] transition flex items-center gap-2"
                                        >
                                            {showAllMembers === i
                                                ? "Lihat Lebih Sedikit"
                                                : "Lihat Selengkapnya"}

                                            <span
                                                className={`transition-transform duration-300 ${showAllMembers === i
                                                    ? "rotate-180"
                                                    : ""
                                                    }`}
                                            >
                                                <ChevronDown size={14} />
                                            </span>
                                        </button>

                                    </div>
                                )}

                            </div>
                        </div>
                    );
                })}
            </div>
        </section>
    );
}