...
export default function HeroSection() {
    ...
    // Ambil foto dari backend untuk dijadikan background tambahan
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const res = await getGaleri("FOTO", 1, 5);
                if (res && res.data && res.data.length > 0) {
                    const fetchedUrls = res.data.map(item => item.url);
                    // Gabungkan default bg dengan foto dari backend
                    setBgImages(["/bg.jpeg", ...fetchedUrls]);
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        }, 3000); // 3 detik (2s nunggu + 1s transisi lebih ideal untuk UX)
